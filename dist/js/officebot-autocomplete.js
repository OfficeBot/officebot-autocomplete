		...
		compile : function() {
			return {
				pre : function(scope, elem, attrs) {
					...
					// if (attrs.contentTemplate) { //itemTemplate represents the interpolated, parsed string
					// 	subTemplate = $(elem).attr('content-template'); //reading the raw attribute gives us the unparsed, which is what we want
					// }
	...
	function linkFn(scope, elem, attrs, ctrl, transclude) {
		...
		***************************************************/
		...
		/**
			...
			* bottom of thiis file (in our scope.$watch listener function for scope.src); We have this stub here in case that
		...
		function searchProxy() {
			return $timeout(function() { return scope.items },0);
  	}
  	...
  	/**
		...
		function onInputKeyup(e) {
			...
			if (e.which === 13) {
				...
				if (currentSelector !== -1) {
					...
					scope.onSubmit().call(this, elem.find('input').val());
					/*
		...
		function mouseoverProxy(item) {
			if ('function' === typeof scope.onMouseover()) {
				scope.onMouseover().call(this, item);
			}
		}
